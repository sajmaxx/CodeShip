...
namespace CigarAndDrinksSelector.Pages
{
    public class CigarsModel : PageModel
    {
        ...
        public void OnGet(string searchCigars)
        {
            CigarsList = string.IsNullOrEmpty(searchCigars)
                ? _cigarData.GetCigarsList()
                : _cigarData.GetCigarsByCompanyName(searchCigars);
        }
    }
}
