...
//      SqlConnection cnnSql = new SqlConnection(cnnString);
//      cnnSql.Open();
//      ResultText = GetConnectionInformation(cnnSql);
//      cnnSql.Close();
//      cnnSql.Dispose();

      ///Avoiding the need to use Close and Dispose, by using the Using clause
      using (SqlConnection cnna = new SqlConnection(cnnString))
      {
          cnna.Open();
          ResultText = GetConnectionInformation(cnna);
