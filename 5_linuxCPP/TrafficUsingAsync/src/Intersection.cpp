...
void AllowEntryForVehicle(promise<void> &&promisedVehicle)
{
   this_thread::sleep_for(chrono::milliseconds(1200));
  promisedVehicle.set_value();
}
...
void WaitingVehicles::permitEntryToFirstInQueue()
{
  ...
  shared_ptr<Vehicle> vechicle = *_vehicles.begin();  
  promise<void> firstvehiclePromise = move(*_promises.begin());
  
  thread backgroundwork(AllowEntryForVehicle, move(firstvehiclePromise));
...
void Intersection::addVehicleToQueue(std::shared_ptr<Vehicle> vehicle)
{
    ...
    promise<void> vehiclePromise;
  	future<void> vehicleFuture = vehiclePromise.get_future(); 
   _waitingVehicles.pushBack(vehicle, move(vehiclePromise)); 
    vehicleFuture.get();
